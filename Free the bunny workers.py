...
def next(l):
    ...
    if n <= 1: #no next
        return None
    nl = [l[i] for i in range(n)]
    oneCount = (1 if nl[n - 1] else 0)
    for i in range(n - 2, -1, -1): #regular bit push
        if nl[i] and (not nl[i + 1]):
            nl[i + 1] = True
            nl[i] = False
            for j in range(oneCount):
                nl[i + 2 + j] = True
            for j in range(i + 2 + oneCount, n):
                nl[j] = False
            return nl
        if nl[i]:
            oneCount += 1
...
def solution(num_buns, num_required):
    ...
    if num_required == 0:
        return ret
    l = [i < n - num_required + 1 for i in range(n)]
    keyIdx = 0
    while l != None:
        for i in range(n):
            if l[i]:
...
'''
Submitting solution...
Submission: SUCCESSFUL. Completed in: 6 mins, 43 secs.
'''
