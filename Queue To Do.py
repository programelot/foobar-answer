...
Output:
    14
    '''
def xorBits(start, length, bit):
    step = 2 ** (bit + 1)#size of cycle
    cnt = 0
    for i in range(length):
        prefix = start + i * length
        #fr, to inclusive
        fr  = prefix
        to  = prefix + (length - i) - 1
        cycleFr = fr//step
        cycleTo = to//step
        if cycleFr == cycleTo:#One cycle
            fr = max(fr, cycleFr * step + step //2)
            if to >= fr:
                cnt += (to + 1 - fr)
            continue
        cnt += (cycleTo - 1 - cycleFr) * (step // 2) #cycle computation
        fr = max(fr, cycleFr * step + step //2)
        cnt += (cycleFr + 1) * step - fr
        if to >= cycleTo * step + step // 2:
            cnt += (to + 1 - cycleTo * step + step // 2)
    return cnt % 2 == 1
