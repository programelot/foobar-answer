...
def count(w, brick):
    ...
    if w * (w + 1) > brick * 2:
        return 0
    if (w, brick) in count_cache:
        return count_cache[(w, brick)]
    ret = 0
    for use in range(1, brick//w + 1):
        if use * w > brick:
            break
        ret += count(w - 1, brick - use * w)
    count_cache[(w, brick)] = ret
    return ret

def solution(n):
    return sum([count(i, n) for i in range(2, 20)])
